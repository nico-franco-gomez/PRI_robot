...
par.add_point_SPTP([0,0,800],initial_rot,25,marker=0)
...
par.add_point_SLIN([0,r,800],[0,rotB(-350),180],0.25,marker=0)
...
if visualize:
    ...
    for n in points:
        ...
        yy.append(n[0][1]/10)
        zz.append(n[0][2]/10)
    ax.scatter(xx,yy,zz,color='xkcd:orange',label='Points de\nmesure')
    ax.set_title(f'Sphère – Rayon {r//10:.0f} cm')
    ax.set_xlabel('$x$ [cm]')
    ax.set_ylabel('$y$ [cm]')
    ax.set_zlabel('$z$ [cm]')
    fig.tight_layout()
